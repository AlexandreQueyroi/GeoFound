...
if (!isset($_SESSION['user'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["disconnect"])) {
        ...
        custom_log("INFO", ' User ' . $_SESSION['user'] . ' with ID : ' . $_SESSION['user_id'] . ' just logged out.', "userConnection.php");
...
custom_log("ERROR", ' User ' . ($_SESSION['user'] ?? 'NULL') . ' with ID : ' . ($_SESSION['user_id'] ?? 'NULL') . " and server variable : " . print_r($_SERVER ?? 'SERVER VARIABLE NOT FOUND', true) . ' just failed connection check.', "userConnection.php");
header("Location: /");
exit;
?>