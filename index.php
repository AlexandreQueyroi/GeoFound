...
if (isset($_POST['envoyer'])) {
    if (!isset($_SESSION['id'])) {
        ...
        try {
            ...
            $stmt = $conn->prepare($sql);
            $stmt->execute([$description, $image_id, $_SESSION['id']]);
...
<div class="flex-grow max-w-4xl mx-auto px-4 py-8">
    <div class="text-black">
        ...
        <button onclick="openModal()" class="bg-[#00272B] text-white p-4 rounded-full fixed bottom-32 right-5 hover:bg-[#003B42] transition">
            ...
            if ($result->rowCount() > 0) {
                while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if (isset($_SESSION['id'])) {
                        ...
                        $stmt->execute([$row['id'], $_SESSION['id']]);
